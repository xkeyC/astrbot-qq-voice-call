...
if ! curl -fsS "http://127.0.0.1:${ASTRBOT_QQ_CALL_AV_HOST_PORT:-6111}/healthz" \
  >/dev/null 2>&1; then
...
unset ASTRBOT_QQ_CALL_AV_HOST ASTRBOT_QQ_CALL_AV_HOST_ENTRY

qq_args=(--no-sandbox)
if [[ -n ${ASTRBOT_QQ_CALL_BOT_UIN:-} ]]; then
  qq_args+=(-q "$ASTRBOT_QQ_CALL_BOT_UIN")
fi
