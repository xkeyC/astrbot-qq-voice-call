...
3. 有近期消息或人物记忆时，电话回复模型生成一句上下文开场白；超时或失败时
   使用固定问候。对方先说话会取消或跳过开场，不阻塞首轮输入。
4. VAD 检测语音并实时追加 ASR 音频。
5. 句尾静音达到阈值后提交 ASR。
6. 非空 ASR 文本直接进入电话回复模型；模型可用 `[WAIT]` 放弃不可靠输入。
7. `ctx.llm.generate` 生成短回复。
8. TTS 首包立即写入 QQ 麦克风。
9. 对方持续说话时终止当前 TTS，并预热下一条 TTS 会话。
